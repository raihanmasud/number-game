...
String letters = "0123456789GAMES"; 
Font font;
Piece blankp;
Piece[] all;
Piece[][] grid;
Random rand;// grid origin in pixels
int gridx;
int gridy;// grid width and height, in cells
int gridw;
int gridh;// cell geometry in pixels


int cellw;
int cellh;
int cellyoff;
int cellxoff;// commands
	 ...
	 if (label != null) {				 
	  if (inv) {    
            // black outlined, white square with black writing
		         g.setColor(0);		             
	 g.setFont(font);		    
         g.drawRect(px, py, cellw-2, cellh-2);		             	 g.drawString(label,px+cellxoff, py+cellyoff,							Graphics.TOP|Graphics.LEFT);
		

		} 


	else {		    // black square with whitewriting
		                g.setColor(0);
	    g.fillRect(px, py, cellw-1, cellh-1);		              g.setColor(0xFFFFFF);		             				g.setFont(font);		    
...
void setGrid(Piece p, int x, int y) 
      		{	
	      grid[x][y] = p;//copy piece type p into piece type grid
		p.setLocation(x, y);
...
void moveBlank(int swapx, int swapy) 
	...
	setGrid(grid[swapx][swapy], blankp.x,blankp.y);	                setGrid(blankp, swapx, swapy);                  			}
...
void randomize_by_moving() 
	...
	while (blankp.y != gridh-1)	               			moveBlank(blankp.x, blankp.y + 1);
...
// shuffle the tiles randomly and place the blank at the bottom right
	...
	void shuffle() 			{	
		...
		for (int i = 0; i < limit; i++) 
			{					
		int j = randRange(limit);						temp = ta[j];			
		ta[j] = ta[i];
		ta[i] = temp;
			}
...
// RATE YOUR MIDP LAN
	...
	swap(0, 3, 2, 3);
		...
		void setState(int ns) {
			...
			gameState = ns;
			switch (gameState) {
	case INITIALIZED:				removeCommand(cmd[CMD_ABOUT]);				addCommand(cmd[CMD_RESET]);				addCommand(cmd[CMD_START]);			removeCommand(cmd[CMD_UNLOCK]);				addCommand(cmd[CMD_EXIT]);				removeCommand(cmd[CMD_OPTIONS]);
			break;

        case PLAYING:
removeCommand(cmd[CMD_ABOUT]);				addCommand(cmd[CMD_RESET]);				addCommand(cmd[CMD_START]);				removeCommand(cmd[CMD_UNLOCK]);				addCommand(cmd[CMD_EXIT]);				removeCommand(cmd[CMD_OPTIONS]);	
		break;	   
...
// create the grid arrays	
		...
		int x = i % gridw;				
		int y = i / gridw;
		String s = letters.substring(i, i+1);			grid[x][y] =all[i] =new Piece(s, i, x, y, i < (gridw*gridh/2));
		}
...
cmd[CMD_ABOUT] 
    =new Command("About", Command.HELP, 5);	


cmd[CMD_EXIT]=new Command("Exit", Command.EXIT,6);

cmd[CMD_OPTIONS] =new Command("Options", Command.SCREEN, 3 );
		

cmd[CMD_START] =new Command("Play", Command.SCREEN, 1); 		
cmd[CMD_RESET] = new Command("Reset", Command.SCREEN, 2);
		
cmd[CMD_UNLOCK] =new Command("Unlock", Command.SCREEN, 4);	

// set up the listener	
setCommandListener(this);	// set up options screen

	//options = new Options(dpy, this);	// set up initial state
	...
	public void commandAction(Command c, Displayable d) 

		{
		 if(c==cmd[CMD_EXIT]) 
			...
			if(c==cmd[CMD_RESET]) 
			{
			cheated = false;							resetGrid();																	setState(INITIALIZED);		
			repaint();		
		...
		if(c==cmd[CMD_START]) 
			...
			if (false) {	//false=options.funny			
			rearrangeFunnily(true);//true=options.true			
			}	 

			else
			{										randomize(true);//true=options.true							     
				}								
			setState(PLAYING);				
			repaint();
		...
		public void paint(Graphics g) {
			g.setColor(0xFFFFFF);
			g.fillRect(0, 0, getWidth(), getHeight());	    	g.translate(gridx, gridy);
		//moves the position of the game box					g.setColor(0);
	g.drawRect(-2, -2, gridw*cellw+2, gridh*cellh+2);
			//Draw The outer boundery of rectangles
	for (int j = 0; j < gridw; j++) 
	...
	if (gameState == WON) {				g.translate(-g.getTranslateX(), -g.getTranslateY());				g.setColor(0);		
	g.setFont(Font.getDefaultFont());		

	g.drawString((cheated ? "CHEATER!" : "YOU WIN!"),		    	getWidth() / 2,getHeight() - 1,		    		Graphics.BOTTOM|Graphics.HCENTER);
...
int swapx = blankp.x;
int swapy = blankp.y;
int direction = 1;//(options.reversed ? -1 : 1);
